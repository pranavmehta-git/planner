...
// Dashboard Page
const DashboardPage = ({ items, courses, reminders, calendarEvents, toggleComplete, onAddItem }) => {
  ...
  return (
    <div>
      ...
      <div className="stats-row">
        ...
        <div className="stat-card">
          <div className="stat-value" style={{ color: 'var(--accent-primary)' }}>{thisWeekItems.length}</div>
        ...
        <div className="card">
          <div className="card-header">
            <div>
              ...
              <p className="card-subtitle">{calendarEvents.filter(e => e.date === new Date().toISOString().split('T')[0]).length} events</p>
            </div>
          </div>
          <div className="event-list">
            {calendarEvents.filter(e => e.date === new Date().toISOString().split('T')[0]).sort((a, b) => a.start.localeCompare(b.start)).map(event => (
        ...
        <div className="card">
          <div className="card-header">
            <div>
              ...
              <p className="card-subtitle">{todayItems.length + todayReminders.length} items</p>
            </div>
          ...
          <div className="task-list">
            {[...todayItems, ...overdueItems].map(item => {
        ...
        <div className="card full-width">
          <div className="card-header">
            <div>
              ...
              <p className="card-subtitle">{thisWeekItems.length} assignments due</p>
            </div>
          </div>
          <div className="task-list">
            {thisWeekItems.sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate)).slice(0, 8).map(item => {
              const course = getCourse(item.courseId);
              return (
...
// Weekly View Page
const WeeklyViewPage = ({ items, courses, toggleComplete, onAddItem, onDeleteItem }) => {
  const getCourse = (courseId) => courses.find(c => c.id === courseId);
  ...
  const groupedByWeek = useMemo(() => {
    ...
    items.filter(item => item.dueDate).forEach(item => {
      const weekNum = getWeekNumber(item.dueDate);
      if (!groups[weekNum]) {
        groups[weekNum] = [];
      }
    ...
    <div>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p className="page-subtitle">All assignments organized by week</p>
        </div>
...
export default function LifeTracker() {
  ...
  const [calendarEvents] = useState(defaultCalendarEvents);
  ...
  // Form states
  const [newItem, setNewItem] = useState({ title: '', type: 'Manual Todo', courseId: '', category: 'Class', dueDate: '', dueTime: '23:59', status: 'Not started', notes: '' });
  const [newCourse, setNewCourse] = useState({ name: '', term: 'Spring 2026', color: '#6366f1', links: '' });
  ...
  const addItem = () => {
    ...
    setNewItem({ title: '', type: 'Manual Todo', courseId: '', category: 'Class', dueDate: '', dueTime: '23:59', status: 'Not started', notes: '' });
  ...
  const addCourse = () => {
    ...
    setNewCourse({ name: '', term: 'Spring 2026', color: '#6366f1', links: '' });
